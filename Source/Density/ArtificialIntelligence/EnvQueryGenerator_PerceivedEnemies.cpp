...
#include "EnvQueryGenerator_PerceivedEnemies.h"
...
#include "Density/Ability/DensAbilityLibrary.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(EnvQueryGenerator_PerceivedEnemies)

#define LOCTEXT_NAMESPACE "EnvQueryGenerator"



static void RemoveDuplicatesSorted(TArray<AActor*>& InOutActors)
{
...
UEnvQueryGenerator_PerceivedEnemies::UEnvQueryGenerator_PerceivedEnemies(const FObjectInitializer& ObjectInitializer) :
...
void UEnvQueryGenerator_PerceivedEnemies::GenerateItems(FEnvQueryInstance& QueryInstance) const
{
	...
	for (AActor* ListenerActor : Listeners)
	{
		...
		if (Listener == nullptr)
		{
			UE_VLOG(QueryOwner, LogEQS, Error, TEXT("Tried to use EnvQueryGenerator_PerceivedEnemies while query context actor %s doesn\'t represent a valid perception listener")
		...
		{
			UE_VLOG(QueryOwner, LogEQS, Error, TEXT("Tried to use EnvQueryGenerator_PerceivedEnemies while query context actor\'s %s UAIPerceptionComponent is missing")
		...
		for (AActor* PerceivedActor : LocalPerceivedActors)
		{
			if (PerceivedActor 
				...
				&& UDensAbilityLibrary::CheckIsEnemy(ListenerActor,PerceivedActor)	))
			{
...
FText UEnvQueryGenerator_PerceivedEnemies::GetDescriptionTitle() const
{
	FFormatNamedArguments Args;
	Args.Add(TEXT("DescribeContext"), UEnvQueryTypes::DescribeContext(ListenerContext));
	return FText::Format(LOCTEXT("DescriptionGeneratePerceivedEnemies", "Enemies perceived by {DescribeContext}"), Args);
};

FText UEnvQueryGenerator_PerceivedEnemies::GetDescriptionDetails() const
{
	FFormatNamedArguments Args;
	...
	if (AllowedActorClass)
	{
		...
		return FText::Format(LOCTEXT("PerceivedEnemiesOfClassDescription", "radius: {Radius}\nsense: {Sense}\nactors of class: {ActorClass}"), Args);
	}

	return FText::Format(LOCTEXT("PerceivedEnemiesDescription", "radius: {Radius}\nsense: {Sense}"), Args);
}
